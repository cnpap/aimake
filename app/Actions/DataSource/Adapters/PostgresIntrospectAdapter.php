...
class PostgresIntrospectAdapter implements DataSourceIntrospectAdapter
{
    ...
    public function listTables(DataSource $dataSource): array
    {
        ...
        $tables = $this->introspectTables($connection, $this->extractSchemas($dataSource));
     ...
     * @return list<TableMetadata>
     */
    protected function introspectTables($connection, ?array $schemas = null): array
    {
        if ($schemas !== null) {
            $schemaBuilder = $connection->getSchemaBuilder();

            return collect($schemaBuilder->getTables($schemas))
                ->map(
     ...
     */
    protected function extractSchemas(DataSource $dataSource): ?array
    {
        $searchPath = $dataSource->data['search_path'] ?? null;
        ...
        $schemas = array_filter(
            array_map(static fn (string $schema): string => trim($schema), explode(',', (string) $searchPath)),
            static fn (string $schema): bool => $schema !== ''
        );

        return $schemas === [] ? null : array_values(array_unique($schemas));
    }
}
