...
import {
    OpenApiToolCard,
    type OpenApiTool,
} from './components/OpenApiToolCard';
...
export default function OpenApiToolsIndex() {
    const [searchQuery, setSearchQuery] = useState('');
    const [activeTab, setActiveTab] = useState<'all' | OpenApiTool['status']>(
        'all',
    );

    const handleCreateSubmit = (data: OpenApiFormValues, close: () => void) => {
        console.log('Registering OpenAPI tool:', data);
        setTimeout(() => {
            close();
        }, 800);
    };
    ...
    const openCreateModal = () =>
        NiceModal.show(FormSheetModal, {
            title: '注册 OpenAPI 工具',
            description: '录入 OpenAPI 文档信息，让 Agent 可以安全调用。',
            children: (close) => (
                <OpenApiForm
    ...
    const filteredTools = openApiTools.filter((tool) => {
        const matchesSearch =
    ...
    return (
        <AppLayout>
            ...
            <CardPage
                ...
                activeTab={activeTab}
                onTabChange={(value) =>
                    setActiveTab(value as 'all' | OpenApiTool['status'])
                }
                searchPlaceholder="搜索 OpenAPI / base URL / 描述..."
                searchValue={searchQuery}
                onSearchChange={setSearchQuery}
                ctaLabel="注册 OpenAPI 工具"
                onCtaClick={openCreateModal}
                items={filteredTools}
                renderItem={(tool) => <OpenApiToolCard tool={tool} />}
                getKey={(tool) => tool.id}
                emptyText="暂无符合条件的 OpenAPI 工具"
                gridMinWidth={340}
            />
        </AppLayout>
    );
